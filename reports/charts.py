...
def horizontal_bar(pdf: FPDF, x: float, y: float, w: float,
                   data: list[tuple[str, int, tuple]], max_val: int | None = None) -> float:
    ...
    Args:
        ...
        max_val: Maximum value for scale (auto-calculated if None)
    
    Returns:
        Height used by the chart
    """
    if not data:
        return 0
    ...
    gap = 2.0
    label_w = 30.0
    bar_area_w = w - label_w - 5.0
    
    if max_val is None:
        max_val = max(val for _, val, _ in data) or 1
    ...
    for i, (label, value, color) in enumerate(data):
        ...
        pdf.set_text_color(*MUTED)
        pdf.set_xy(x, bar_y)
        pdf.cell(label_w, bar_h, label[:15], align="R")
        
        # Bar background
        pdf.set_fill_color(*SURFACE2)
        pdf.rect(x + label_w + 2, bar_y, bar_area_w, bar_h, "F")
        
        # Bar fill
        fill_w = (value / max_val) * bar_area_w if max_val > 0 else 0
        pdf.set_fill_color(*color)
        pdf.rect(x + label_w + 2, bar_y, fill_w, bar_h, "F")
        
        # Value label
        pdf.set_font(pdf.font_family, "B", 7)
        pdf.set_text_color(*INK)
        pdf.set_xy(x + label_w + bar_area_w + 3, bar_y)
        pdf.cell(15, bar_h, str(value), align="R")
