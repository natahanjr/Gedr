...
class GedrPDF(FPDF):
    ...
    def __init__(self, *, classification: str = "", report_id: str = "",
        ...
        self.section_no = 0
        self._toc_entries: list[tuple[str, int]] = []  # (title, page_no)
    ...
    def open_section(self, title: str, *, numbered: bool = True, new_page: bool = True) -> str:
        ...
        self._toc_entries.append((self.current_section, self.page_no() + 1))
        if new_page:
            self.add_page()
    ...
    def table_of_contents(self) -> None:
        """Generate a table of contents page from recorded section entries."""
        self.add_page()
        y = MARGIN_T
        self.set_xy(MARGIN_L, y)
        self.sans("B", TYPE.h1)
        self.set_text_color(*NAVY)
        self.cell(0, TYPE.h1 * 0.62 + 1.6, "Table of Contents", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.ln(2)
        self.set_draw_color(*HAIRLINE)
        self.set_line_width(0.25)
        self.line(MARGIN_L, self.get_y(), PAGE_W - MARGIN_R, self.get_y())
        ...
        for title, page_no in self._toc_entries:
            self.ensure_space(8)
            y = self.get_y()
            ...
            self.set_text_color(*INK)
            self.set_xy(MARGIN_L, y)
            self.cell(140, 5.5, self._disp(title))
            # Dotted leader
            self.set_text_color(*FAINT)
            dots = "\u00b7 " * 40
            self.set_xy(MARGIN_L + 140, y)
            self.cell(24, 5.5, dots[:30])
            # Page number
            self.sans("B", 9.5)
            self.set_text_color(*NAVY)
            self.set_xy(PAGE_W - MARGIN_R - 16, y)
            self.cell(16, 5.5, str(page_no), align="R")
            self.set_y(y + 7)
